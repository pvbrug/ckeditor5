	...
	describe( 'createDropdown()', () => {
		...
		describe( 'has default behavior', () => {
			describe( 'closeDropdownOnClickOutside()', () => {
				...
				it( 'uses `getOutsideDropdownElements` to determine if the click was outside the dropdown', () => {
					...
					const getOutsideDropdownElementsSpy = sinon.spy( () => [ buttonA ] );

					dropdownView.element.remove();
					dropdownView = createDropdown( locale, undefined, {
						getOutsideDropdownElements: getOutsideDropdownElementsSpy
					} );
					...
					sinon.assert.calledTwice( getOutsideDropdownElementsSpy );
			...
			describe( 'closeDropdownOnBlur()', () => {
				...
				it( 'uses `getOutsideDropdownElements` to determine if the blur was outside the dropdown', async () => {
					const getOutsideDropdownElementsSpy = sinon.spy( () => [ global.document.activeElement ] );
					...
					expect( dropdownView.isOpen, 'isOpen' ).to.be.true;
					expect( getOutsideDropdownElementsSpy ).to.be.calledTwice;
				} );
